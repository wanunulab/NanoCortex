# Regional nanopore signal plotting (Claude Code)
...
Confirm only that the paths exist, BAM indexes are available, `start < end`, and the requested contig occurs in both BAMs. Execute with the Bash tool:
...
- `<SKILL_DIR>`: `~/.claude/skills/nanocortex` or project `.claude/skills/nanocortex`
...
Report the genomic interval, sample roles, reads cap, y-axis range, output path, and Bash exit code.
