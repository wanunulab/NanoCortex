# Regional nanopore signal plotting (Codex)
...
Confirm only that the paths exist, BAM indexes are available, `start < end`, and the requested contig occurs in both BAMs. Then run through the Codex shell tool:
...
- `<SKILL_DIR>`: `$CODEX_HOME/skills/nanocortex`
...
Report the genomic interval, sample roles, reads cap, y-axis range, output path, and exit code.
