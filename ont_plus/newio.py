...
def get_io_reads(
    bam_reads, pod5_dr, reverse_signal=False, missing_ok=True, pa_scaling=None
):
...
def plot_ref_region_reads(
    ...
    xlab="Reference Position",
    ylab="Normalized Signal",
    ref_mod=None,  # ✅ 新增参数：修饰位置 tuple 或 list
):
    """Plot read signals over reference region"""

    if sample_names is None:
        sample_names = [
            f"Sample{samp_idx}" for samp_idx in range(len(samples_read_ref_regs))
        ]
    ...
    if ylim is None:
        ...
        # ✅ 修复：保证 sig_min, sig_max 始终定义
        sig_min, sig_max = ylim
    ...
    # 生成坐标 + 碱基信息
    # -----------------------------
    base_positions = np.arange(ref_reg.start, ref_reg.end)
    base_coords = pl.DataFrame(
        [base_positions, seq],
        schema=["pos", "base"],
        orient="col",
    ...
    if ref_mod is not None:
        mod_set = set(ref_mod)
        base_colors = ["red" if pos in mod_set else "black" for pos in base_positions]
    else:
        base_colors = [BASE_COLORS[b] for b in seq]
    ...
    # ✅ 高亮区域绘制（必须在 sig_min/max 已定义后）
    if highlight_ranges is not None and len(highlight_ranges) > 0:
        highlight_df = pl.DataFrame(
            highlight_ranges,
            schema=["start", "end", "color"],
            orient="row",
        ).with_columns(
            pl.lit(sig_min).alias("sig_min"),
            pl.lit(sig_max).alias("sig_max"),
        )
        p = (
            p
            + p9.geom_rect(
    ...
    # avg_df = (
    #     sig_df.groupby(["Sample", "Reference Position"])
    #     .agg(pl.col("Signal").mean().alias("MeanSignal"))
    # )

    # p = (
    #     p
    #     + p9.geom_line(
    #         p9.aes(x="Reference Position", y="MeanSignal", color="Sample"),
    #         data=avg_df,
    #         size=1.2,
    #         alpha=0.9,
    #     )
...
def plot_signal_at_ref_region(
    ...
    sample_names=None,
    sample_colors=None,
    mean_line_colors=None,
    **kwargs,
):
    """Plot signal from reads at a reference region, and save as PDF.
    ...
    Args:
        ...
        sample_names (list[str] or None): Sample display names for plotting
        sample_colors (list[str] or None): Colors for each sample for plotting (signal colors)
        mean_line_colors (list[str] or None): Colors for each sample's mean line
        **kwargs: Passed on to plot_ref_region_reads

    Returns:
        The absolute path of the saved PDF file.
    """
    ...
    seq, _ = get_ref_seq_and_levels_from_reads(
        ref_reg, chain(*reg_bam_reads), sig_map_refiner
    )

    if sample_names is not None:
        sample_names_to_use = list(sample_names)
    else:
        sample_names_to_use = [f"Sample{idx}" for idx in range(len(samples_read_ref_regs))]

    # 获取trimmean值来计算每个base位置的mean值
    samples_metrics, _ = get_ref_reg_samples_metrics(
        ref_reg,
        pod5_bam_pairs,
        sig_map_refiner=sig_map_refiner,
        skip_sig_map_refine=skip_sig_map_refine,
    ...
    # 计算每个样本在每个base位置的mean值
    region_len = ref_reg.end - ref_reg.start
    mean_signals = []
    for sample_metrics in samples_metrics:
        if "trimmean" in sample_metrics:
            mean_signal = np.nanmean(sample_metrics["trimmean"], axis=0)
            mean_signals.append(mean_signal)
        else:
            mean_signals.append(np.full(region_len, np.nan))

    # 创建横线数据：每个base位置一条横线
    mean_line_entries = []
    for sidx, mean_signal_array in enumerate(mean_signals):
        sample_name = sample_names_to_use[sidx]
        line_color = mean_line_colors[sidx] if mean_line_colors is not None and sidx < len(mean_line_colors) else None
        for pos_idx, mean_value in enumerate(mean_signal_array):
            if not np.isnan(mean_value):
                base_pos = ref_reg.start + pos_idx
                entry = {
                    "Sample": sample_name,
                    "x": base_pos,
                    "xend": base_pos + 1,
                ...
                mean_line_entries.append(entry)
    
    mean_line_df = pl.DataFrame(mean_line_entries) if mean_line_entries else pl.DataFrame()

    base_plot = plot_ref_region_reads(
        ref_reg,
        samples_read_ref_regs,
        seq,
        None,
        ylim=ylim,
        ref_mod=ref_mod,
        sample_names=sample_names_to_use,
        sample_colors=sample_colors,
        **kwargs,
    )

    if not mean_line_df.is_empty():
        mean_line_pd = mean_line_df.to_pandas()
        if mean_line_colors is not None:
            for sidx, sample_name in enumerate(sample_names_to_use):
                if sidx < len(mean_line_colors):
                    sample_data = mean_line_pd[mean_line_pd["Sample"] == sample_name]
                    if not sample_data.empty:
                        mean_lines = p9.geom_segment(
                            p9.aes(x="x", xend="xend", y="y", yend="y"),
                            data=sample_data,
                            color=mean_line_colors[sidx],
                            size=1.0,
                            alpha=0.8
                        )
                        base_plot = base_plot + mean_lines
        else:
            mean_lines = p9.geom_segment(
                p9.aes(x="x", xend="xend", y="y", yend="y", color="Sample"),
                data=mean_line_pd,
                size=1.0,
                alpha=0.8
            )
    ...
    # Save as PDF to save_data_path
    base_plot.save(save_data_path, format="pdf")
    return os.path.abspath(save_data_path)
...
class Read:
    ...
    def into_remora_read(self, use_reference_anchor):
        ...
        if use_reference_anchor:
            # print(f"ref_to_signal: {self.ref_to_signal}")
            # print("cigar: ", self.cigar)
            # print("ref_seq: ", self.ref_seq)
            # print(f"read_id: {self.read_id}")
            if self.ref_to_signal is None:
